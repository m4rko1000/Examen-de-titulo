...
(function ($) {
  function init(plot) {
    ...
    function fixData(data) {
      for (var i = 0; i < data.length; ++i) {
        if (typeof data[i].data == "number") data[i].data = [[1, data[i].data]];
        else if (
    ...
    function draw(plot, newCtx) {
      ...
      function drawPie() {
        ...
        function drawSlice(angle, color, fill) {
          ...
          ctx.beginPath();
          if (Math.abs(angle - Math.PI * 2) > 0.000000001) ctx.moveTo(0, 0);
        ...
        function drawLabels() {
          ...
          else var radius = maxRadius * options.series.pie.label.radius;
